import{r as t,u as _,p as u,j as e,a as A,b as p}from"./index-BxtmL2gZ.js";const E=()=>{const[f,i]=t.useState(!1),[g,b]=t.useState([]),[N,v]=t.useState([]),{token:d,logout:m}=_(),h={id:"123",city:"Kota"},[a,j]=t.useState({name:"",email:"",mobile:"",message:"",category_id:"",class_id:"",enquiry_for:""});t.useEffect(()=>{y(),k()},[]);const y=()=>{i(!0),u(p.getCategory,{},d,m).then(({success:s,data:l})=>{s&&b(l)}).catch(console.log).finally(()=>i(!1))},k=()=>{i(!0),u(p.getClassesList,{},d,m).then(({success:s,data:l})=>{s&&v(l)}).catch(console.log).finally(()=>i(!1))},c=s=>{const{name:l,value:n}=s.target,r=l==="mobile"?n.replace(/\D/g,"").slice(0,10):n;j(x=>({...x,[l]:r}))},w=async s=>{s.preventDefault();const{name:l,email:n,mobile:r,message:x,category_id:C,class_id:q,enquiry_for:S}=a;if(!l||!n||!r||!x||!C||!q||!S){alert("Please fill in all required fields.");return}if(!/^\d{10}$/.test(r)){alert("Mobile number must be a valid 10-digit number.");return}try{i(!0);const o=await u(p.saveContactPageRequest,a,d,m);o.success?(alert("Request submitted successfully!"),j({name:"",email:"",mobile:"",message:"",category_id:"",class_id:"",enquiry_for:""})):alert(o.message||"Something went wrong.")}catch(o){console.error("Submit Error:",o),alert("Failed to submit request.")}finally{i(!1)}};return e.jsx(e.Fragment,{children:e.jsxs("section",{children:[e.jsx("style",{children:`
