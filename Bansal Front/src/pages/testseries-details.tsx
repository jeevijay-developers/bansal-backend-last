...
import TestSeriesTabs from "../components/TestSeriesTabs";
...
const TestSeriesDetails = () => {
  ...
  const [otherTestSeries, setOtherTestSeries] = useState([]);
  ...
  const getTestSeriesDetails = async (incomingSlug?: string) => {
  setLoading(true);
  try {
    const slugToUse = incomingSlug || slug;

    const resp = await postApi(
      APIPATH.testSeriesDetail,
      { slug: slugToUse },
      token,
      logout
    );
    ...
    setOtherTestSeries(data.otherTestSeries);
    setExams(data.exams);
  } catch (error: any) {
  ...
  const getCouponList = () => {
    ...
    postApi(
      APIPATH.couponList,
      { user_id: userDetails?.id, type: "test" },
      token,
      logout
    )
      .then((resp) => {
          ...
          otherTestSeries.map((test: any) => (
            <div key={test.id} className="col-lg-4 col-md-6 mb-4">
              <div className="test-series-card">
                ...
                >
                  <div className="test-series-card-top">
                    ...
                    <p className="test-series-card-badge">
                      ₹{test?.offer_price ? test?.offer_price : test?.price}
                    </p>
                  </div>
               ...
               <button
  onClick={() => handleViewDetails(test?.slug)}
