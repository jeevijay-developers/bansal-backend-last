  ...
  const fetchExams = async () => {
    ...
    try {
      const resp = await postApi(
        APIPATH.courseExamListApi,
        { course_id: decodedNumber },
        token,
        logout
      );
          ...
          <div className="container">
            <div className="course-study-top-align">
              <div>
                {testSeries && (
                  <>
                    <h1 className="sub-heading mb-2">
                      {testSeries.course_name}
                    </h1>
                    <p className="text mb-0 text-secondary">
                      Course Pratice Test
                    </p>
                  </>
                )}
              ...
              <div className="container">
                <ul className="test-series-price-nav-list">
                  {examList.map((test) => (
                    <li
                      key={test.id}
                      className="test-serieslist-card test-study-card1"
                    >
                      <div className="d-flex justify-content-between">
                        <h4>{test.test_name}</h4>
                        <div className="tslc-specific-tags">
                          <span>
                            Start Time : End Time : {test.start_date_time || 0}
                          </span>
                          <span>
