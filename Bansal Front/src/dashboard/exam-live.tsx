...
{questionData?.question_type === "boolean" && (
      ...
      {["True", "False"].map((value) => (
        <React.Fragment key={value}>
          <input
            type="radio"
            className="btn-check"
            name={`boolean-answer-${questionData?.id}`}
            id={`boolean-${value}-${questionData?.id}`}
            autoComplete="off"
            checked={studentAnswer === value}
            onChange={() => {
              if (originalAnswer == null && studentAnswer !== null) {
          ...
          <label
            className={`btn btn-outline-primary ${
              studentAnswer === value ? "active" : ""
            }`}
            htmlFor={`boolean-${value}-${questionData?.id}`}
          >
            {value}
          </label>
        </React.Fragment>
      ...
      previousId === "" && (
        <div className="text-primary">
          <strong>Your Previous Answer</strong>
                  ...
                  {questionData?.question_type === "integer" && (
  <div className="my-5">
    <label htmlFor={`integer-answer-${questionData?.id}`}>Enter Answer</label>
    <input
      type="number"
      className="form-control w-100"
      id={`integer-answer-${questionData?.id}`}
      min="0"
      max="99"
      value={studentAnswer || ""}
      onChange={(e) => {
  ...
  {["single_choice", "multiple_choice"].includes(questionData?.question_type) && (
    <div>
      <input
        type={questionData?.question_type === "single_choice" ? "radio" : "checkbox"}
        className="btn-check"
        name={`options-outlined-${questionData.id}`} // Make name unique per question for radio
        id={`success-outlined-${optionKey}`}
        autoComplete="off"
        checked={
          questionData?.question_type === "single_choice"
            ? selectedOption === optionKey
            : selectedOption?.split(",").includes(optionKey)
        }
        onChange={() => {
          if (questionData?.question_type === "single_choice") {
            ...
            let selectedArray = selectedOption ? selectedOption.split(",") : [];

            if (selectedArray.includes(optionKey)) {
              selectedArray = selectedArray.filter((item) => item !== optionKey); // uncheck
            } else {
              selectedArray.push(optionKey); // check
            }

            const updated = selectedArray.join(",");
            setSelectedOption(updated);
            setStudentAnswer(updated);
          }
        }}
      />
      ...
      >
        <div className="d-flex justify-content-between align-items-center w-100">
          <div className="d-flex align-items-center w-100" style={{ gap: "5px" }}>
            ...
            {questionData?.[`option${optionKey}`]?.includes("<img") ? (
              <p
                dangerouslySetInnerHTML={{
                  __html: questionData?.[`option${optionKey}`],
                }}
              />
            ) : (
              <MathJax dynamic>{questionData?.[`option${optionKey}`]}</MathJax>
            )}
          </div>
